# Auto-adapted from CPython Lib/test/test_heapq.py
# Tests fastpy's ability to compile and run the heapq module
# Stdlib source inlined from: C:\Users\inhah\AppData\Local\Python\pythoncore-3.13-64\Lib\heapq.py

# ======================================================================
# Inlined stdlib module: heapq
# ======================================================================
...
# [stripped C-extension import from line 582]
try:
...
# Helper methods from TestHeapPython
def check_invariant(heap):
    for pos, item in enumerate(heap):
        if pos:
            parentpos = pos - 1 >> 1
            assertTrue(heap[parentpos] <= item)
...
# Test functions from TestHeapPython
def TestHeapPython__test_heappushpop():
    h = []
    x = heappushpop(h, 10)
    assertEqual((h, x), ([], 10))
    h = [10]
    x = heappushpop(h, 10.0)
    assertEqual((h, x), ([10], 10.0))
    assertEqual(type(h[0]), int)
    assertEqual(type(x), float)
    h = [10]
    x = heappushpop(h, 9)
    assertEqual((h, x), ([10], 9))
    h = [10]
    x = heappushpop(h, 11)
    assertEqual((h, x), ([11], 10))

def TestHeapPython__test_heappop_max():
    h = [3, 2]
    assertEqual(_heappop_max(h), 3)
    assertEqual(_heappop_max(h), 2)

def TestHeapPython__test_empty_merges():
    assertEqual(list(merge([], [])), [])
    assertEqual(list(merge([], [], key=lambda: 6)), [])
...
try:
    TestHeapPython__test_heappushpop()
    print("TestHeapPython.test_heappushpop: PASS")
except Exception as _e:
    print("TestHeapPython.test_heappushpop: FAIL -", _e)
try:
    TestHeapPython__test_heappop_max()
    print("TestHeapPython.test_heappop_max: PASS")
except Exception as _e:
    print("TestHeapPython.test_heappop_max: FAIL -", _e)
try:
    TestHeapPython__test_empty_merges()
    print("TestHeapPython.test_empty_merges: PASS")
except Exception as _e:
    print("TestHeapPython.test_empty_merges: FAIL -", _e)