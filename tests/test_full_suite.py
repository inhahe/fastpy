...
5. **CPython adapted tests** (full differential: compile + compare output)
   - tests/cpython_adapted/ — hand-written + auto-generated from CPython's test suite
   - Includes auto-generated stdlib-inlined tests (test_*_stdlib.py)
   - Deterministic output — full stdout comparison with CPython.
...
Run:
    ...
    pytest tests/test_full_suite.py -k cpython_adapted # just CPython adapted tests
"""
...
# ---------------------------------------------------------------------------
# 5. CPython adapted tests — hand-written + auto-generated from CPython's
#    test suite.  Includes both language-feature tests and stdlib-inlined
#    tests (test_*_stdlib.py) generated by tests/cpython_adapter/.
# ---------------------------------------------------------------------------

_CPYTHON_ADAPTED_DIR = Path(__file__).resolve().parent / "cpython_adapted"

_CPYTHON_ADAPTED_FILES = sorted(
    p for p in _CPYTHON_ADAPTED_DIR.glob("test_*.py")
) if _CPYTHON_ADAPTED_DIR.exists() else []

# Import the xfails from the dedicated test module so they stay in sync
from tests.test_cpython_adapted import _XFAILS as _CPYTHON_ADAPTED_XFAILS


@pytest.mark.parametrize(
    "test_file",
    _CPYTHON_ADAPTED_FILES,
    ids=[p.stem for p in _CPYTHON_ADAPTED_FILES],
)
def test_cpython_adapted(test_file: Path):
    """Compile an adapted CPython test and diff output against CPython."""
    stem = test_file.stem
    if stem in _CPYTHON_ADAPTED_XFAILS:
        pytest.xfail(_CPYTHON_ADAPTED_XFAILS[stem])

    result = _diff_test_file_direct(test_file, timeout=60.0)
    if result.skipped:
        pytest.skip(result.reason)
    if result.failed:
        pytest.fail(result.detail())
